...
cd exegete
python src/lookup.py "John 3:16"     # 바로 작동 (WEB 영어 성경 기본 포함)
```

**원어 추가** (헬라어·히브리어 형태소, 약 100MB, CC BY):
```bash
python setup_data.py
```

**개역개정 등 다른 번역본 사용** (저작권 준수하여 본인이 준비):
```bash
# data/bible_krv.txt 넣기 (한 줄=한 절: "창1:1 태초에 하나님이...")
